...
find . -mindepth 2 -maxdepth 3 \
  ...
  -not -path "*/.git/*" \
  -not -path "*/vendor/*" \
  \( -name go.mod -o -name pyproject.toml -o -name setup.py \
     -o -name package.json -o -name tsconfig.json -o -name Cargo.toml \) \
  2>/dev/null | while read -r marker; do
...
```bash
[[ -d .claude/rules ]] && find .claude/rules -maxdepth 1 -name "*.md" 2>/dev/null | while read -r f; do echo "rules:$f"; done
true
```
...
find . -mindepth 2 -maxdepth 3 -name "CLAUDE.md" -path "*/.claude/CLAUDE.md" 2>/dev/null | while read -r f; do
  echo "legacy:${f#./}"
done
true
```
